...
const Languages = () => {
  ...
  const language_list =
    dictionary && dictionary[tsv] ? Object.keys(dictionary[tsv]) : [];
  useEffect(() => {
    if (!_.isUndefined(lang)) {
      if (language_list.includes(lang)) dispatch(changeLanguage(lang));
    }
  }, [lang, language_list]);

  const keywordByLang = (language) => {
    return dictionary && dictionary[tsv] && dictionary[tsv][language]
      ? dictionary[tsv][language]["lang_label"]
      : "";
  };
    ...
    <div>
      ...
      >
        {language_list.map((key) => {
          return (
            <MenuItem key={key} onClick={() => handleCloseItem(key)}>
              {" "}
              {keywordByLang(key)}{" "}
            </MenuItem>
          );
