    ...
    return (
        <Accordion expanded={histoVisible} onChange={() => setHistoVisible(!histoVisible)}>
            <AccordionSummary
                ...
                id={"panel0a-header"}
            >
                <Typography className={classes.heading}>{keyword(state.result.histogramInsta.title)}</Typography>
            </AccordionSummary>
            <AccordionDetails>
                {}
                <div style={{ width: '100%', }}>
                    {(state.result.histogramInsta.json && (state.result.histogramInsta.json.length === 0) &&
                        <Typography variant={"body2"}>{keyword("twittersna_no_data")}</Typography>)}
                    {(state.result.histogramInsta.json && state.result.histogramInsta.json.length !== 0) &&
                    <Plot useResizeHandler
                        style={{ width: '100%', height: "450px" }}
                        data={state.result.histogramInsta.json}
                        layout={state.result.histogramInsta.layout}
                        config={state.result.histogramInsta.config}
