  ...
  return (
    <Paper className={classes.root}>
      ...
      {request &&
        ...
        result &&
        result.pieCharts && <PlotPieChart result={result} request={request} />}
      {request &&
        request.userList &&
        request.userList.length === 0 &&
        result &&
        result.tweetCount && <BubbleChart result={result} request={request} />}
      {<HeatMap result={result} request={request} />}
      {request && <HashtagGraph result={result} request={request} />}
      {request && <SocioSemGraph result={result} request={request} />}
      {request && <CloudChart result={result} request={request} />}
      {request && <GexfExport result={result} request={request} />}
      <Box m={3} />
      {result.urls && <UrlList result={result} request={request} />}
      {request && <Clusters result={result} request={request} />}
    </Paper>
  );
}
