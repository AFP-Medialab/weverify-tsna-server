...
export function getAggregationData(param) {
  ...
  let query = JSON.stringify(buildQuery(aggs, must, mustNot, 0, false))
    .replace(/\\/g, "")
    .replace(/"{/g, "{")
    .replace(/}"/g, "}");
...
export async function getTweets(param) {
  ...
  return queryTweetsFromES(aggs, must, mustNot, cloudTweets)
...
async function queryTweetsFromES(aggs, must, mustNot, cloudTweets) {
  const response = await fetch(elasticSearch_url, {
    method: "POST",
    body: JSON.stringify(buildQuery(aggs, must, mustNot, 10000, cloudTweets))
      .replace(/\\/g, "")
      .replace(/"{/g, "{")
      .replace(/}"/g, "}"),
    headers: {
  ...
  let isMore10kRecords =
    elasticResponse["hits"]["total"]["value"] === 10000 &&
    elasticResponse["hits"]["total"]["relation"] === "gte"
      ? true
      : false;
  ...
  if (isMore10kRecords) {
    do {
      ...
      elasticResponse = await continueQueryTweetsFromESWhenMore10k(
        aggs,
        must,
        mustNot,
        searchAfter,
...
async function continueQueryTweetsFromESWhenMore10k(
  aggs,
  must,
  mustNot,
  searchAfter,
...
//Build a query for elastic search
function buildQuerySearchAfter(
  aggs,
  must,
  mustNot,
  size,
...
function constructMatchNotPhrase(param) {
  if (param.bannedWords === null || param.bannedWords === undefined) return [];
  else {
    let match_phrases = "";
...
function constructAggs(param) {
  ...
  let aggs = {
    ...
    top_user_retweet: constructAggForTop(
      "screen_name",
      "desc",
      20,
      "sum",
    ...
    top_user_mentions: constructAggForTop(
...
function buildQueryMultipleMatchPhrase(field, arr) {
  ...
  let query =
    '{ "size": 10000,  "sort": [ { "indexedat": "desc" } ] ,"query": { "bool": { "should": [' +
    match_phrases +
    " ] } } }";
  return query;
}
...
export function getESQuery4Gexf(param) {
  ...
  const userAction = async () => {
    ...
    while (
      !(gexfStatus.status === "COMPLETED" || gexfStatus.status === "FAILED")
    ) {
      await timer(3000);
      const statusResp = await fetch(gexfStatus_url, {
        //check gexf status
        method: "POST",
    ...
    let gexfResults = [];
    if (
      gexfStatus.status === "COMPLETED" &&
      gexfResponse.gexfStatus.message?.length
    ) {
...
export function getESQuery4TweetSimilarity(param) {
  ...
  const userAction = async () => {
    const response = await fetch(tweetSimClusterURL, {
      method: "POST",
      credentials: "include",
