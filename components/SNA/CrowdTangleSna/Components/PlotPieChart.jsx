...
export default function PlotPieChart(props) {
  ...
  useEffect(() => {
    setfilesNames(
      "PlotChart_" + "_" + "props.request.from" + "_" + "props.request.until",
    );
  ...
  if (typer == "INSTA") {
    ...
    onDonutsClick = (data, index) => {
      ...
      if (index === 3) {
        ...
        let filteredTweets = state.result.data
          .filter(
            (tweet) =>
              tweet.account !==
              undefined /*&& tweet.account.length > 0 && tweet.account.length !== null*/,
          )
          .filter(function (tweet) {
            return (
              tweet.account /*.toLowerCase()*/ ===
      ...
      // For retweets, likes, top_user donut; typeof condition to avoid error when click on the center
        ...
        let dataToDisplay =
          index === 0
            ? displayPostsInsta(filteredTweets, keyword, "retweetNb")
            : index === 1
            ? displayPostsInsta(filteredTweets, keyword, "nbLikes")
            : displayPostsInsta(filteredTweets, keyword);
    ...
    onDonutsClick = (data, index) => {
      ...
      if (index === 3) {
        ...
        let filteredTweets = state.result.data
          .filter(
            (tweet) =>
              tweet.page_name !== undefined && tweet.page_name.length > 0,
          )
          .filter(function (tweet) {
      ...
      // For retweets, likes, top_user donut; typeof condition to avoid error when click on the center
        ...
        let dataToDisplay =
          index === 0
            ? displayPostsFb(filteredTweets, keyword, "retweetNb")
            : index === 1
            ? displayPostsFb(filteredTweets, keyword, "nbLikes")
            : displayPostsFb(filteredTweets, keyword);
        //    console.log("DATA_DISPLAY ",dataToDisplay)
  ...
  const onDonutsClick1 = (data, index) => {
    ...
    if (index === 3) {
      ...
      let filteredTweets = state.result.data
        .filter(
          (tweet) => tweet.account !== undefined && tweet.account.length > 0,
        )
        .filter(function (tweet) {
      ...
      <div key={index}>
        <Card>
          <div style={{ position: "relative" }}>
            ...
            {obj.json !== null &&
            !(
              obj.json[0].values.length === 1 && obj.json[0].values[0] === ""
            ) ? (
              <CustomCardHeader
                title={obj.title}
                showHelp={true}
