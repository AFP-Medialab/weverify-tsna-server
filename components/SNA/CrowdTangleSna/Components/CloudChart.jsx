...
export default function cloudChart(props) {
  ...
  const CSVheaders = [
    { label: keyword("ct_sna_result_word"), key: "word" },
    { label: keyword("ct_sna_result_nb_occ"), key: "nb_occ" },
    { label: keyword("ct_sna_result_entity"), key: "entity" },
  ];

  function getCSVData() {
    if (!props.result.cloudChart.json) return "";
    let csvData = props.result.cloudChart.json.map((wordObj) => {
      return {
        word: wordObj.text,
        nb_occ: wordObj.value,
  ...
  const getCallbacks = () => {
    return {
      ...
      getWordTooltip: (word) => tooltip(word),
  ...
  function filterTweetsGivenWord(word) {
    ...
    let filteredTweets = state.result.data
      .filter(
        (tweet) =>
          tweet.description !== undefined && tweet.description !== null,
      )
      .map((tweet) => {
    ...
    let filteredTweets1 = state.result.data
      .filter(
    ...
    if (type === "FB") {
      ...
      let filteredTweets3 = state.result.data
        .filter(
          (tweet) => tweet.message !== undefined && tweet.message !== null,
        )
        .map((tweet) => {
      ...
      for (var i = 0; i < length1.length; i++) {
        if (filteredTweets3[i] !== undefined && filteredTweets3[i] !== null) {
          if (
            filteredTweets3[i].includes(word) === true ||
            filteredTweets3[i].includes(word) === true
          ) {
    ...
    for (var i = 0; i < length1.length; i++) {
      if (
        filteredTweets1[i] !== undefined &&
        filteredTweets[i] !== undefined &&
        filteredTweets1[i] !== null &&
        filteredTweets[i] !== null
      ) {
        if (
          filteredTweets[i].includes(word) === true ||
          filteredTweets1[i].includes(word) === true
        ) {
          if (filteredTweets2.includes(state.result.data[i]) === false) {
            filteredTweets2.push(state.result.data[i]);
          }
  ...
  return (
    <Card>
      {props.result &&
        props.result.cloudChart &&
        props.result.cloudChart.json &&
        props.result.cloudChart.json.length !== 0 && (
          <CustomCardHeader
            ...
            {props.result.cloudChart &&
              ...
              props.result.cloudChart.json.length !== 0 && (
                <div id="top_words_cloud_chart" height={"100%"} width={"100%"}>
                  <ReactWordcloud
                    key={JSON.stringify(props.result)}
                    options={props.result.cloudChart.options}
                    callbacks={call}
                    words={props.result.cloudChart.json}
                  />
                  <Box m={1} />
                </div>
              )}
