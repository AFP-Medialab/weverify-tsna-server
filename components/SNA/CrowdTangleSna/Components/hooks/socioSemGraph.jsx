...
function getTweetAttrObjArr(tweets /*,topUser*/) {
  ...
  if (tweets[0].facebook_id) {
    ...
    tweetAttrObjArr = tweets.map((tweet) => {
      ...
      var hashtags = [];
      if (
        tweet.description != null ||
        tweet.description != undefined ||
        tweet.image_text != null ||
        tweet.image_text != undefined ||
        tweet.message != null ||
      ...
      if (
        ...
        if (
          ...
          userIsMentioned = userIsMentioned1
            .concat(userIsMentioned2)
            .concat(userIsMentioned3);
        } else {
        ...
        if (userIsMentioned != null || userIsMentioned != undefined) {
          for (var i = 0; i < userIsMentioned.length; i++) {
            userIsMentioned[i] = userIsMentioned[i].replace(
              /[^@._A-Za-z0-9]/g,
              "",
            );
    ...
    tweetAttrObjArr = tweets.map((tweet) => {
      ...
      var post = [];
      if (
        tweet.total_interactions != null ||
        tweet.total_interactions != undefined
      ) {
...
function getTopNodeGraph(
  graph,
  sortByProp = ["size"],
  topByType = [20, 20, 20, 20],
  types = ["Hashtag", "Mention", "URL", "USERS"],
) {
...
const createSocioSemantic4ModeGraph = (tweets /*, topUser*/) => {
  ...
  let topNodeGraph = getTopNodeGraph(
    ...
    ["size"],
    [20, 20, 20 /*, 20, 20, 20*/],
    ["Hashtag", "Mention", "URL" /*, 'USERS' ,'RetweetWC', 'TopRT'*/],
  );
