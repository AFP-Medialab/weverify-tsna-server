...
function getNodesAsHashtag(data) {
  let nodes = getUniqValuesOfField(
    data,
    "description",
    "image_text",
    "message",
...
function getEdgesCoHashtag(data) {
  ...
  let coHashtagArr2 = data
    .filter(
      (tweet) => tweet.image_text !== undefined && tweet.image_text !== null,
    )
    .map((tweet) => {
      return tweet.image_text;
    });
  var coHashtagArr = [];
  if (data[0].facebook_id) {
    let coHashtagArr3 = data
  ...
  nodeIds.forEach((arr) => {
    for (let i = 0; i < arr.length - 1; i++) {
      for (let j = i + 1; j < arr.length; j++) {
        ...
        edges.push({
          id: sortedVertices.join("___and___"),
...
function groupByThenSum(
  ...
  arrOfObjects.reduce((res, value) => {
    if (!res[value[key]]) {
      let obj = {};
      obj[key] = value[key];
      if (attrToSkip.length > 0 && attrToSkip.length !== null) {
        attrToSkip.forEach((attr) => {
          obj[attr] = value[attr];
        });
      }
      if (attrToSumStr.length > 0 && attrToSumStr.length !== null) {
        attrToSumStr.forEach((attr) => {
          obj[attr] = "";
        });
      }
      if (attrToSumNum.length > 0 && attrToSumNum.length !== null) {
