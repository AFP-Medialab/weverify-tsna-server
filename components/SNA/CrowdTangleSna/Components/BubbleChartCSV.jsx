    ...
    if (result.data[0].facebook_id) {
      ...
      filteredTweets = state.result.data.filter(function (tweetObj) {
        if (isNumeric(tweetObj.page_name) === false) {
          return (
            tweetObj.page_name.toLowerCase() === selectedUser.toLowerCase()
          );
        } else {
          return (
            tweetObj.page_name.toString().toLowerCase() ===
            selectedUser.toString().toLowerCase()
          );
        }
      });
      ...
      filteredTweets = state.result.data.filter(function (tweetObj) {
        if (isNumeric(tweetObj.user_name) === false) {
          return (
            tweetObj.user_name.toLowerCase() === selectedUser.toLowerCase()
          );
        } else {
          return (
            tweetObj.user_name.toString().toLowerCase() ===
            selectedUser.toString().toLowerCase()
          );
        }
      });
      ...
      <AccordionDetails>
        {
          ...
          <div style={{ width: "100%" }}>
            {[
              createBubbleChartOfMostActiveUsers(
                /*topUserProfile,*/ props.result,
                keyword,
              ),
            ].map((bubbleChart) => {
              return (
