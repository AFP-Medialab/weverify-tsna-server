...
export default function cloudChart(props) {
  ...
  const CSVheaders = [
    { label: keyword("twittersna_result_word"), key: "word" },
    { label: keyword("twittersna_result_nb_occ"), key: "nb_occ" },
    { label: keyword("twittersna_result_entity"), key: "entity" },
  ];
  ...
  const tooltip = (word) => {
    ...
    else return "The word " + word.text + " appears " + word.value + " times.";
  };
  ...
  function filterTweetsGivenWord(word) {
    let filteredTweets = props.result.tweets.filter(function (tweetObj) {
      return tweetObj._source.full_text
        .toLowerCase()
        .match(
          new RegExp(
            "(^|((.)*[.()0-9!?'’‘\":,/\\%><«» ^#]))" +
