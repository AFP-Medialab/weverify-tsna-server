...
function getUniqValuesOfField(tweets, field) {
  let nodeIds = tweets
    .filter((tweet) => tweet._source[field] !== undefined)
    .map((tweet) => {
      return tweet._source[field];
    })
    .flat();
...
function getNodesAsHashtag(tweets) {
  let nodes = getUniqValuesOfField(tweets, "hashtags").map((val) => {
...
function getEdgesCoHashtag(tweets) {
  let coHashtagArr = tweets
    .filter(
      (tweet) =>
        tweet._source.hashtags !== undefined &&
        tweet._source.hashtags.length > 1,
    )
    .map((tweet) => {
