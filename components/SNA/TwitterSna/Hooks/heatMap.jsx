...
export function displayTweets(filteredTweets, keyword, sortedColumn) {
  ...
  if (sortedColumn === "nbLikes") {
    columns = [
      ...
      {
        title: "twittersna_result_tweet",
        field: "tweet",
        render: getTweetWithClickableLink,
      },
      {
        title: "twittersna_result_like_nb",
        field: "nbLikes",
        defaultSort: "desc",
      },
    ...
    columns = [
      ...
      {
        title: "twittersna_result_retweet_nb",
        field: "retweetNb",
        defaultSort: "desc",
      },
    ];
  ...
  filteredTweets.forEach((tweetObj) => {
    ...
    resData.push({
      ...
      screen_name: (
        <a
          href={"https://twitter.com/" + tweetObj._source.screen_name}
          target="_blank"
...
function getNbTweetsByHourDay(dayArr, hourArr, bucket) {
  ...
  let nbTweetArr2D = [...Array(dayArr.length)].map((e) =>
    Array(hourArr.length).fill(0),
  );
