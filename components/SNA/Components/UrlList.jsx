...
export default function UrlList(props) {
  ...
  function goToTwitterSnaWithUrlSearch(event, rowData) {
    ...
    for (let obj in rowData) {
      ...
      window.open(
        "/pluginredirect" +
          "?data=" +
          encodeURIComponent(JSON.stringify(newReq)) +
          "&user=" +
          userData,
        "_blank",
      );
  ...
  function createCSVFromURLTable(urls) {
    ...
    urls.data.forEach((row) => (csvArr += row.url + "," + row.count + "\n"));
    return csvArr;
  }
  ...
  return (
    <Card>
      <CustomCardHeader
        title={"13. " + keyword(props.title_message)}
        showHelp={true}
        helpText={"twittersna_urls_tip"}
        showCSV={true}
        functionCSV={() =>
          downloadClick(
            props.request,
            createCSVFromURLTable(props.result.urls),
            "Urls",
            false,
            "",
          )
